...
| **M3** | ~~**`/health/agent` is unauthenticated and in-memory only.**~~ **RESOLVED 2026-07-29** — optional `HEALTH_CHECK_TOKEN` shared-secret gate (falls back to open when unset, so local dev/CI can't break); `keepalive.yml` sends it from a new GH Actions secret. Setting the token on Render + as a GH secret is a remaining dashboard step, tracked in `PLAN.md` §6. The in-memory/resets-on-restart nature is unchanged (documented limitation, not fixed — see H2's boot recovery for the closest mitigation). | `api/health.js:16`, `domain/agentWorkerStatus.js` | Minor information disclosure, plus the monitoring blindness in C2 — a restarted or cold instance always reports healthy. |
...
| **M5** | ~~**The Render sleep mitigation rests on a single GitHub Actions cron.**~~ **PARTIALLY RESOLVED 2026-07-29** — cron tightened `*/10`→`*/5`, `curl --retry` added so a transient blip doesn't count as a missed ping. Honestly not fully closed by code: true redundancy against GitHub Actions itself being unavailable needs an independent, non-GitHub watchdog (free UptimeRobot/cron-job.org) — needs a dashboard account, tracked in `PLAN.md` §6, not silently marked done. | `.github/workflows/keepalive.yml` | ADR-0007's entire answer to free-tier sleep has no redundancy and fails silently. `RENDER_APP_URL` is still unset, so it currently no-ops with a warning. |
...
| **M8** | ~~**Consent version pinned at 1 while a new data use was added.**~~ **RESOLVED 2026-07-29** — `CURRENT_CONSENT_VERSION` bumped to 2, plus a 5th disclosure added to `ConsentPage.jsx` covering usage analytics. Every existing student must re-consent before their next mic-enabled room. | `domain/consent.js:9`, `apps/web/src/lib/analytics.js` | Currently inert — key unset, posthog-js dead-code-eliminated. Becomes a live DPDP problem the moment `VITE_POSTHOG_KEY` is set, because existing students would never be asked to re-consent. |
| **M9** | ~~**Unbounded reads.**~~ **RESOLVED 2026-07-29** — explicit `LIMIT` added to all four: `listQueue` (500), `listRoomIdsForUser`/`listRoomsByIds` (200 each), `listTranscriptLinesForRoom` (5000). Not verified live against the real Supabase project this session (no DB access) — same caveat as any other `db/` change. | `db/matchmakingQueue.js`, `db/roomParticipants.js`, `db/rooms.js` | Harmless at 20–30 students. Degrades predictably as history accumulates — a heavy user's `/api/history/mine` grows without bound. |
...
| ID | Finding | Location |
|---|---|---|
| **L1** | ~~README is the placeholder `"# This is the readme"`.~~ **RESOLVED 2026-07-29** — real README: what this is, repo layout, dev setup (folds in L7), doc pointers. | `Readme.md` |
| **L2** | ~~Dead weight tracked in the repo: `spike/` (21 files, own lockfile and `.env.example`), an empty `.agents/`, and a stray `stitch_placeme_stitch_ui_overhaul (1)/`.~~ **RESOLVED 2026-07-29** — `spike/` removed (superseded by `apps/server` since Phase 1). `.agents/` and the stray folder were already gone by the time this ran. | `spike/`, `.agents/` |
| **L3** | ~~`packages/shared` exists solely to `export {}`~~ **RESOLVED 2026-07-29** — removed; root `package.json`'s workspaces no longer globs `packages/*`. | `packages/shared` |
| **L4** | ~~Dead return value: `matchmake()` computes `remainingQueue`, which no caller reads~~ **RESOLVED 2026-07-29** — removed. | `domain/matchmaking.js:27` |
| **L5** | ~~`recordDispatchSuccess(roomId)` accepts and discards `roomId`~~ **RESOLVED 2026-07-29** — now tracked as `lastSuccess: { roomId, at }`, exposed on `GET /health/agent`. | `domain/agentWorkerStatus.js:20` |
| **L6** | ~~LiveKit tokens minted with `name: req.userId`, putting a raw UUID in the participant name.~~ **RESOLVED 2026-07-29** — now uses the participant's display name (via `listProfilesFn`), falling back to the user id when no profile exists. | `api/rooms.js:80` |
| **L7** | ~~Environment friction: nvm default is Node 20...~~ **RESOLVED 2026-07-29** — folded into L1's new README, which now states the Node 22 + `nvm use` requirement up front. The "4 commits behind" half was a transient state observation from whenever the audit was written, not an ongoing issue. | `.nvmrc`, `.npmrc` |
| **L8** | ~~**Uncommitted work sitting in the working tree**...~~ **Already resolved before this session** — verified: the `beforeunload` guard and both host configs were committed via an intervening commit (`2d2b0ac`) before this 2026-07-29 session started. No action needed here. | `apps/web/src/pages/LobbyPage.jsx`, `apps/web/vercel.json`, `apps/web/public/_redirects` |
