...
| `0014_rooms_max_participants.sql` | ❌ **Not yet applied** — code merged to `dev` (PR #73, 2026-07-31) | BE-2 (`place-me-UI/docs/BACKEND_REQUIREMENTS.md`, `SPEC-0002`): adds `rooms.max_participants integer not null default 6` + a `CHECK(3-12)` constraint. **Do not promote `dev` → `main` before applying this migration to the live project** — `POST /api/rooms` would 500 on the missing column otherwise. |
| `0015_rooms_visibility.sql` | ❌ **Not yet applied** — code merged to `dev` (PR #74, 2026-07-31) | BE-3 (`place-me-UI/docs/BACKEND_REQUIREMENTS.md`, `SPEC-0003`): adds `rooms.visibility text not null default 'private'` + a `CHECK(public/private)` constraint. **Do not promote `dev` → `main` before applying this migration to the live project** — `POST /api/rooms` would 500 on the missing column otherwise. Apply alongside `0014`/`0016`/`0017` if all are still pending at the same time. |
| `0016_rooms_level.sql` | ❌ **Not yet applied** — code merged to `dev` (PR #76, 2026-08-01) | BE-4 (`place-me-UI/docs/BACKEND_REQUIREMENTS.md`, `SPEC-0005`), room-creation half only: adds `rooms.level text not null default 'intermediate'` + a `CHECK` constraint. **Do not promote `dev` → `main` before applying this migration to the live project.** |
| `0017_feedback_structured_output.sql` | ❌ **Not yet applied** — code merged to `dev` (PR #77, 2026-08-01) | BE-6/BE-7 (`place-me-UI/docs/BACKEND_REQUIREMENTS.md`, `SPEC-0006`): adds `feedback.score integer` (+ a 0–100 `CHECK`), `dimensions`/`strengths`/`improvements jsonb not null default '[]'`. **Do not promote `dev` → `main` before applying this migration to the live project** — `insertFeedback` would fail on the missing columns otherwise. No RLS change (feedback still has no client-writable policy). Guardrail #1 also applies: a real human must confirm actual generated feedback reads as sensible and non-discouraging before this ships to `main`. |
| `0018_profiles_college_graduation_year.sql` | ❌ **Not yet applied** — code on unmerged branch `feat/be-14-signup-profile-fields` | BE-14 (`place-me-UI/docs/BACKEND_REQUIREMENTS.md`, `SPEC-0008`): adds `profiles.college text`, `profiles.graduation_year int` (both nullable), extends the `handle_new_user()` signup trigger to read them. **Unlike every other migration in this table, omitting it doesn't break anything already working** — signup keeps functioning identically until it's applied. |
...
target from BE-4 onward is `dev` only** — `dev` → `main` promotion is
being held until confidence is higher and stays entirely the user's own
call (per direct user instruction, 2026-08-01; doesn't change anything
already true per `BRANCHING.md`, just restated for this phase).
