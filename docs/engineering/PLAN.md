...
| 🔶 | **BE-14** | Signup profile fields (college, grad year) | `feat/be-14-signup-profile-fields` | Code complete (`SPEC-0008`), migration `0018` **not yet applied live** — see §3, but uniquely among this batch, omitting it doesn't break anything already working. Not a JS/Express change at all — a Postgres trigger (`handle_new_user()`) extension; no unit test exists, verification is a live-Supabase integration test (confirmed RED against the current live schema — "column profiles.college does not exist" — expected until the migration lands). Not yet merged/PR'd. |
