...
| 🔶 | **BE-6/BE-7** | Numeric overall score + per-dimension rubric | `feat/be-6-be-7-feedback-structured-output` | Picked up directly per user instruction (P1, out of the original P0 order). Code complete (`SPEC-0006`): Gemini feedback prompt reworked to structured JSON (score, 5-dimension rubric, strengths, improvements) via Gemini's `responseSchema` mode; `GET /api/rooms/:id/feedback/mine` and `GET /api/history/mine` return the new fields additively. Migration `0017` **not yet applied live** — see §3, blocks `main` promotion. place-me-UI's `ended.$roomId.tsx` wiring (replacing the `feedbackScores`/`feedbackStrengths`/`feedbackImprovements` mocks) done in the same pass — see place-me-UI's own history. Rebasing onto current `dev` now (this branch forked before BE-1/BE-4 merged) before its own PR. **Guardrail #1's human-verification gate (a real session's feedback, read by a human, still non-discouraging) is outstanding — not to be called done on tests alone.** |
