...
| `0015_rooms_visibility.sql` | ❌ **Not yet applied** — code on unmerged branch `feat/be-3-room-visibility` | BE-3 (`place-me-UI/docs/BACKEND_REQUIREMENTS.md`, `SPEC-0003`): adds `rooms.visibility text not null default 'private'` + a `CHECK(public/private)` constraint. **Do not merge this branch to `main` before applying this migration to the live project** — `POST /api/rooms` would 500 on the missing column otherwise. Apply alongside `0014` if both are still pending at the same time. |
