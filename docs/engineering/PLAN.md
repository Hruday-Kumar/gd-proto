...
| `0015_rooms_visibility.sql` | ❌ **Not yet applied** — code merged to `dev` (PR #74, 2026-07-31) | BE-3 (`place-me-UI/docs/BACKEND_REQUIREMENTS.md`, `SPEC-0003`): adds `rooms.visibility text not null default 'private'` + a `CHECK(public/private)` constraint. **Do not promote `dev` → `main` before applying this migration to the live project** — `POST /api/rooms` would 500 on the missing column otherwise. Apply alongside `0014`/`0016` if all are still pending at the same time. |
| `0016_rooms_level.sql` | ❌ **Not yet applied** — code on unmerged branch `feat/be-4-room-level` | BE-4 (`place-me-UI/docs/BACKEND_REQUIREMENTS.md`, `SPEC-0005`), room-creation half only: adds `rooms.level text not null default 'intermediate'` + a `CHECK` constraint. **Do not merge this branch to `main` before applying this migration to the live project.** |
...
| 🔶 | **BE-4** | Room level (room-creation half only) | `feat/be-4-room-level` | Code complete (`SPEC-0005`), migration `0016` **not yet applied live** — see §3. Not yet merged/PR'd. Match-side level filtering deferred to fold into BE-5 (per direct user instruction — too big/risky to bundle with a single-table field: needs a `matchmaking_queue` schema change + a change to `matchmake()` itself). |
