...
| `0017_feedback_structured_output.sql` | ❌ **Not yet applied** — code on unmerged branch `feat/be-6-be-7-feedback-structured-output` | BE-6/BE-7 (`place-me-UI/docs/BACKEND_REQUIREMENTS.md`, `SPEC-0006`): adds `feedback.score integer` (+ a 0–100 `CHECK`), `dimensions`/`strengths`/`improvements jsonb not null default '[]'`. **Do not merge this branch before applying this migration to the live project** — `insertFeedback` would fail on the missing columns otherwise. No RLS change (feedback still has no client-writable policy). Guardrail #1 also applies: a real human must confirm actual generated feedback reads as sensible and non-discouraging before this ships to `main`. |
...
| 🔶 | **BE-10** | Per-participant talk-time share (post-session half) | `feat/be-10-talk-time-share` | Code complete (`SPEC-0007`), **no migration needed** — `transcript_lines` already had `started_at_ms`/`ended_at_ms`. Not yet merged/PR'd. Live/mid-session half (`ParticipantTile`'s live badge, `/session`'s speak-time card) is BE-17, not attempted. |
