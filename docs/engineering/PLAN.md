...
| `0014_rooms_max_participants.sql` | ✅ **Confirmed applied — user-confirmed live, 2026-08-02** | BE-2 (`place-me-UI/docs/BACKEND_REQUIREMENTS.md`, `SPEC-0002`): adds `rooms.max_participants integer not null default 6` + a `CHECK(3-12)` constraint. Taken on the user's word ahead of PR #83's `dev` → `main` promotion, same as `0012`'s pattern — not independently re-verified against the live schema this session (no live DB access here). |
| `0015_rooms_visibility.sql` | ✅ **Confirmed applied — user-confirmed live, 2026-08-02** | BE-3 (`place-me-UI/docs/BACKEND_REQUIREMENTS.md`, `SPEC-0003`): adds `rooms.visibility text not null default 'private'` + a `CHECK(public/private)` constraint. Taken on the user's word, same as `0014`. |
| `0016_rooms_level.sql` | ✅ **Confirmed applied — user-confirmed live, 2026-08-02** | BE-4 (`place-me-UI/docs/BACKEND_REQUIREMENTS.md`, `SPEC-0005`), room-creation half only: adds `rooms.level text not null default 'intermediate'` + a `CHECK` constraint. Taken on the user's word, same as `0014`. |
| `0017_feedback_structured_output.sql` | ✅ **Confirmed applied — user-confirmed live, 2026-08-02** | BE-6/BE-7 (`place-me-UI/docs/BACKEND_REQUIREMENTS.md`, `SPEC-0006`): adds `feedback.score integer` (+ a 0–100 `CHECK`), `dimensions`/`strengths`/`improvements jsonb not null default '[]'`. No RLS change (feedback still has no client-writable policy). Taken on the user's word, same as `0014`. **Guardrail #1 still applies independently of the migration** — a real human must confirm actual generated feedback reads as sensible and non-discouraging; that check is not satisfied merely by the column existing (see §6). |
| `0018_profiles_college_graduation_year.sql` | ✅ **Confirmed applied — user-confirmed live, 2026-08-02** | BE-14 (`place-me-UI/docs/BACKEND_REQUIREMENTS.md`, `SPEC-0008`): adds `profiles.college text`, `profiles.graduation_year int` (both nullable), extends the `handle_new_user()` signup trigger to read them. Merged to `dev` via PR #80, 2026-08-01. Taken on the user's word, same as `0014`. |
...
BE-3's `visibility` column to exist first), then BE-4 (room-creation half
only) as the first P1, then BE-6/BE-7 picked up directly per user
instruction, ahead of BE-5/BE-8/BE-9/BE-10/BE-14/BE-19. **Every merge
target from BE-4 through BE-9 was `dev` only** — `dev` → `main` promotion
was held until confidence was higher and stayed entirely the user's own
call (per direct user instruction, 2026-08-01). **That call was made
2026-08-02**: migrations `0014`–`0018` confirmed applied live (see §3),
and the batch promoted via PR #83.

| Status | ID | Task | Branch | Notes |
|---|---|---|---|---|
| ✅ | **BE-2** | Configurable room capacity | `feat/be-2-room-capacity` | Merged to `dev` via PR #73, 2026-07-31. Migration `0014` **confirmed applied live, 2026-08-02** — see §3. |
| ✅ | **BE-3** | Room visibility (public/private) | `feat/be-3-room-visibility` | Merged to `dev` via PR #74, 2026-07-31 (`pr-review`'d, one general blocker-free approve). Migration `0015` **confirmed applied live, 2026-08-02** — see §3. |
| ✅ | **BE-1** | Room discovery (`GET /api/rooms/open`) | `feat/be-1-room-discovery` | Merged to `dev` via PR #75, 2026-08-01. **No migration needed** — both `visibility`/`max_participants` already existed. All three agreed P0s now merged. |
| ✅ | **BE-4** | Room level (room-creation half only) | `feat/be-4-room-level` | Merged to `dev` via PR #76, 2026-08-01. Migration `0016` **confirmed applied live, 2026-08-02** — see §3. Match-side level filtering deferred to fold into BE-5 (per direct user instruction — too big/risky to bundle with a single-table field: needs a `matchmaking_queue` schema change + a change to `matchmake()` itself). |
| ✅ | **BE-6/BE-7** | Numeric overall score + per-dimension rubric | `feat/be-6-be-7-feedback-structured-output` | Merged to `dev` via PR #77, 2026-08-01 (rebased onto current `dev` first — forked before BE-1/BE-4 merged). Gemini feedback prompt reworked to structured JSON (score, 5-dimension rubric, strengths, improvements) via Gemini's `responseSchema` mode; `GET /api/rooms/:id/feedback/mine` and `GET /api/history/mine` return the new fields additively. Migration `0017` **confirmed applied live, 2026-08-02** — see §3. **Guardrail #1's human-verification gate (a real session's feedback, read by a human, still non-discouraging) is outstanding — not to be called done on tests alone**, independent of the migration being live. |
| ✅ | **BE-19** | "Analyzed" badge → real score | — (frontend-only, `place-me-UI`) | Done, not yet live. `place-me-UI`'s `HistorySession` type + both `toSessionRow` mappers (`history.tsx`, `index.tsx`) now pass through the real `score` BE-6/BE-7 added — turned out not to be fully automatic as this item's own doc originally predicted, needed explicit wiring. No `gd-proto` code change. Migration `0017` now live, so real numbers should render instead of `null`. |
| ✅ | **BE-10** | Per-participant talk-time share (post-session half) | `feat/be-10-talk-time-share` | Merged to `dev` via PR #79, 2026-08-01. **No migration needed** — `transcript_lines` already had `started_at_ms`/`ended_at_ms`. Live/mid-session half (`ParticipantTile`'s live badge, `/session`'s speak-time card) is BE-17, not attempted. |
| ✅ | **BE-14** | Signup profile fields (college, grad year) | `feat/be-14-signup-profile-fields` | Merged to `dev` via PR #80, 2026-08-01. Migration `0018` **confirmed applied live, 2026-08-02** — see §3. Not a JS/Express change at all — a Postgres trigger (`handle_new_user()`) extension. |
| ✅ | **BE-8** | Score trend over time | — (frontend-only, `place-me-UI`) | Done, not yet live. Entirely client-side, no `gd-proto` code — `ProgressChart` gained an optional `series` prop, both `history.tsx`/`index.tsx` compute a last-8-scored-sessions trend from `GET /api/history/mine`'s already-fetched response. Migration `0017` (BE-6/BE-7) now live, so real trend data should render instead of the empty state. |
| ✅ | **BE-9** | Aggregate stats (streak, avg score, speak-time%) | `feat/be-9-aggregate-stats` | Code complete (`SPEC-0009`), **no migration needed**. `GET /api/history/mine` gained per-session `talkShare` (batched transcript fetch across the whole history, reusing BE-10's aggregation) since BE-10 alone only covered one room's participants, not the caller's own cross-history share. "Avg. score"/"Streak" needed no backend change. Promoted to `main` via PR #83, 2026-08-02. |
