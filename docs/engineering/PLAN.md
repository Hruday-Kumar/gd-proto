...
**Last updated:** 2026-07-29 (N2 — DONE. Migration `0013` applied by the
user and live-verified: the `topics` RLS bypass and the missing length
constraint are both closed, confirmed via a live re-run of the new
`topicsRlsIsolation.test.js` on Node 22, before-and-after. This finding is
from `AUDIT_COMPARISON_2026-07-29.md`, not the 2026-07-28 `AUDIT.md` this
file otherwise tracks. Previous update: N7 verified — migration `0011`
confirmed applied to the live project via a live re-run of the existing H8
RLS isolation test. Before that: N1 fixed — feedback generation now
retries with backoff instead of failing permanently and silently. See
`PROGRESS.md` for all session records.)
...
```
npx vitest run --root apps/server    # 274 tests, all must pass (2 new N2 unit tests; a 3rd new live-RLS test file skips offline)
